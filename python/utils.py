...
def plotBinaryMap(axes, name, data, colors=['black','lime','gray']):
  ...
  for i in range(plotW):
    ...
    if len(arr)<plotW:
      arr=np.concatenate([arr, np.ones(plotW-len(arr))*10])# fill end where is nothing
    rf[:, i] = arr
...
def plotEnvironment(axes, name, env, agentPos , colors=['white','blue','red','lightGray','cyan']):
	...
	print(arr)
	print(cm)
	axes.set_xticks(np.arange(-.5, 20, 1))
	axes.set_yticks(np.arange(-.5, 20, 1))
